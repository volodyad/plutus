...
handleBalanceTx UnbalancedTx{unBalancedTxTx, unBalancedTxUtxoIndex} = do
    ...
    inputValues <- traverse (lookupValue unBalancedTxUtxoIndex) (Set.toList $ Tx.txInputs unBalancedTxTx)
    collateral  <- traverse (lookupValue unBalancedTxUtxoIndex) (Set.toList $ Tx.txCollateral unBalancedTxTx)
    let fees = txFee unBalancedTxTx
        left = txForge unBalancedTxTx <> fold inputValues
    ...
    tx' <- if Value.isZero pos
           then do
               ...
               pure unBalancedTxTx
                ...
                pure $ addOutputs ownPubKey pos unBalancedTxTx
    ...
    tx'' <- if Value.isZero neg
            then do
                ...
                addInputs utxo ownPubKey neg tx'
